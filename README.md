...
### Обработка документов НПА
```python
# Пример обработки приказа Минздрава
unified = UnifiedPDFProcessor()
result = unified.process_file_for_rag("приказ_минздрав_123.pdf")

# Результат содержит:
print(f"Ведомство: {result['metadata']['ministry']}")
print(f"Тип документа: {result['metadata']['document_type']}")  
print(f"Номер: {result['metadata']['doc_number']}")
print(f"Дата: {result['metadata']['date']}")
```
...
chunks = text_splitter.split_text(rag_result['text'])
...
vectorstore = Chroma.from_texts(
    ...
    metadatas=[rag_result['metadata']] * len(chunks)
)
```
...
**Версия:** 2.1.0  
**Лицензия:** MIT  
**GitHub:** https://github.com/Andrew821667/Extract_Processor
